...
describe('Cart e2e test', () => {
  ...
  it('should load Carts', async () => {
    ...
    expect(await cartComponentsPage.getTitle()).to.eq('Carts');
  });

  it('should load create Cart page', async () => {
    await cartComponentsPage.clickOnCreateButton();
    cartUpdatePage = new CartUpdatePage();
    expect(await cartUpdatePage.getPageTitle()).to.eq('Create or edit a Cart');
  ...
  it('should delete last Cart', async () => {
    ...
    expect(await cartDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this Cart?');
